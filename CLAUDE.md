...
> **任何代码、数据库、配置的改动，必须经过用户明确同意后才能执行。** 不要在未经确认的情况下自行修改文件、执行 SQL、或推送代码。

> **优先使用 CLI 工具，而非让用户手动操作。** 对于有 CLI 能力的产品（Supabase CLI、Railway CLI、GitHub CLI 等），应当直接通过 CLI 执行操作，而不是要求用户去 Dashboard 手动点击。
...
> **用户反馈问题时，禁止立即做最小/最快修复。** 表面问题背后往往隐藏更深层的逻辑链路、更多的代码牵涉。正确做法是：① 完整复盘问题背后的真实原因；② 分析所有受影响的代码路径；③ 给出若干可选方案（包含利弊对比）；④ 由用户决定采用哪个。随意选择最简单的方案往往会破坏项目整体性。
